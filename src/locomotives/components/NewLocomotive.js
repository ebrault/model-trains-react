...
class NewLocomotive extends React.Component {
  ...
  async componentDidMount() {
    const response = await axios.get(apiUrl + '/locomotives/' + `${this.state.match.params.id}`, { headers: { 'Content-Type': 'application/json', 'Authorization': `Token token=${this.state.user.token}` } } )
    this.setState({locomotive: response.data.locomotive})
  }

  handleChange = (event) => {
    const newLocomotive = {...this.state.locomotve, [event.target.name]: event.target.value}
  ...
  handleSubmit = async (event, user) => {
    ...
    await axios.post(apiUrl + '/locomotives', locomotiveParams, { headers: { 'Content-Type': 'application/json', 'Authorization': `Token token=${this.state.user.token}` } } )
      ...
      <React.fragment>
        <div className = "IndexLocomotives">
          ...
          <form>
            ...
            <button type='submit' onClick={(event) => this.handleSubmit(event, user)}>Add</button>
          </form>
        </div>
      </React.fragment>
    )
